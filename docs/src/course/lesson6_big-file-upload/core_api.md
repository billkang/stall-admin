...
### 1.1 简介
`FileReader` 是 HTML5 提供的核心 API，用于在浏览器端异步读取文件内容。它允许开发者通过 JavaScript 直接操作用户选择的文件（如图片、文档等），并将其转换为可处理的格式（如二进制数据、文本或 Base64 编码的字符串）。其核心特性包括：  
...
reader.readAsBinaryString(blob) // 已弃用，返回原始二进制字符串（兼容旧浏览器）
...
|-------------------|-----------------------------|------------------------|-------------|
| 小文件预览        | `readAsDataURL`             | 图片缩略图生成          | ```javascript const img = new Image(); img.src = reader.result; ``` |
| 文本解析          | `readAsText`                | CSV 文件解析            | ```javascript const csvLines = result.split('\n'); ``` |
| 二进制处理        | `readAsArrayBuffer`         | 文件分片、哈希计算      | ```javascript const buffer = new Uint8Array(result); ``` |
| 兼容性处理        | `readAsBinaryString`        | 旧版 IE 浏览器支持      | ```javascript // 仅在低版本浏览器中使用 ``` |
...
|------------------|----------|--------------------|----------|----------|
| `memoryStorage`  | 高       | 小文件快速处理     | 快       | ```javascript const upload = multer();``` |
| `diskStorage`    | 低       | 大文件安全存储     | 中等     | ```javascript const upload = multer({ storage: multer.diskStorage(...) });``` |
| `s3Storage`      | 无       | 云原生应用（AWS）  | 依赖网络 | ```javascript const s3Storage = require('multer-s3');``` |
| `customStorage`  | 可变     | 特殊需求（如加密） | 自定义   | ```javascript const customStorage = { _handleFile: ... };``` |
...
const hybridStorage = multer.diskStorage({
    _handleFile(req, file, cb) {
        if (file.size > 100 * 1024 * 1024) { // 大于 100MB 使用磁盘
            multer.diskStorage._handleFile(req, file, cb);
        } else { // 小文件暂存内存
...
|------------------------|----------------------------|-------------------------------|------------------------------|----------|
| **FileReader + Stream** | 浏览器端大文件分片          | 内存占用低，支持断点续传      | 需处理分片顺序和哈希校验     | 视频平台上传（1GB+ 文件）    |
| **Multer + diskStorage** | 传统表单文件上传            | 配置简单，开箱即用            | 不支持超大文件（需分片）     | 企业级表单提交（<500MB）    |
| **Stream + 云存储SDK**  | 企业级云原生应用            | 无缝对接云服务，扩展性强      | 依赖网络稳定性，成本较高     | 日志实时上传至 AWS S3       |
