...
静态路由和动态路由都会走权限控制，可以通过配置路由的 `meta` 属性中的 `authority` 字段来控制权限，可以参考[路由权限控制](https://github.com/stalljs/stall-admin/blob/main/playground/src/router/routes/modules/demos.ts)。
...
const routes: RouteRecordRaw[] = [
  {
    ...
    meta: {
      ...
      icon: STALL_LOGO_URL,
      order: 9999,
      title: $t('page.stall.title'),
    },
    name: 'StallProject',
    path: '/stall-admin',
    redirect: '/stall-admin/about',
    children: [
      {
        name: 'StallAbout',
