...
有了权限码，就可以使用 `@stall/access` 提供的`AccessControl`组件及API来进行按钮的显示与隐藏。

#### 组件方式

```vue
<script lang="ts" setup>
import { AccessControl, useAccess } from '@stall/access';
...
```vue
<script lang="ts" setup>
import { AccessControl } from '@stall/access';
</script>
...
```vue
<script lang="ts" setup>
import { useAccess } from '@stall/access';
