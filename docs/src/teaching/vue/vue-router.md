    ...
    for (const guard of beforeResolveGuards.list()) {
      ...
      if (result === false)
        return Promise.reject(
          new Error('Navigation aborted by beforeResolve guard'),
        );
    }
...
export const RouterView = defineComponent({
  ...
  setup(props, { attrs, slots }) {
    ...
    return () => {
      ...
      return h(ViewComponent, { ...routeProps, ...attrs });
    };
  },
});
```
...
export const RouterLink = defineComponent({
  ...
  setup(props, { slots }) {
    ...
    // 从路由器实例中注入配置选项
    const {
      options: { linkActiveClass, linkExactActiveClass },
    } = inject(routerKey);
    ...
    const elClass = computed(() => ({
      // 动态生成激活状态的类名
      [getLinkClass(activeClass, linkActiveClass, 'router-link-active')]:
        link.isActive,
      // 动态生成精确激活状态的类名
      [getLinkClass(
        exactActiveClass,
...
export function createWebHistory(base = '') {
  ...
  const { pauseListeners, listen, destroy } = useHistoryListeners(
    base,
    state,
    location,
    replace,
...
- 状态管理：通过 history.state 管理路由状态，支持导航方向的判断。createWebHashHistory 的核心作用是确保 base 的格式正确，并将实际的路由逻辑委托给 createWebHistory。
