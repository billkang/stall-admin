...
import { isBoolean, isObject, isUndefined } from '@stall-lowcode/utils';
...
export function getAccessTarget(scope: RuntimeScope, accessType: AccessTypes): Record<string, unknown> {
  switch (accessType) {
...
export function addToScope(
  ...
  if (accessType === AccessTypes.PROPS && Object.keys(source).length > 0) {
    ...
    if (Object.keys(propsOptions).length > 0) {
      instance.propsOptions = [{ ...rawPropsOptions, ...propsOptions }, [...rawNeedCastKeys, ...needCastKeys]];
    }
  }
}
