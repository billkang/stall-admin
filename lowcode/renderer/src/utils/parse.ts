...
export class SchemaParser {
  ...
  constructor(options?: SchemaParserOptions) {
    this.createFunction =
      options && !options.thisRequired
        ? code => new Function('__exports__', '__scope__', `with(__exports__) { with(__scope__) { ${code} } }`)
  ...
  initModule(schema: IPublicTypeContainerSchema) {
    ...
    const res = initModuleSchema ? this.parseSchema(initModuleSchema, false) : initModuleSchema;
  ...
  parseOnlyJsValue(schema: unknown): unknown {
    if (isJSExpression(schema) || isJSFunction(schema) || isI18nData(schema)) {
      return undefined;
    } else if (isArray(schema)) {
  ...
  parseExpression(str: JSExpression | JSFunction, scope?: RuntimeScope | boolean): CallableFunction | unknown;
  parseExpression(str: JSExpression | JSFunction, scope?: RuntimeScope | boolean): CallableFunction | unknown {
    try {
