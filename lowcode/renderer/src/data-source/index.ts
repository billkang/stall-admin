import type { InterpretDataSource, InterpretDataSourceConfig } from '@stall-lowcode/types';
...
import { isPlainObject, isUndefined, isBoolean, isFunction } from '@stall-lowcode/utils';

const same = <T>(v: T) => v;
const noop = () => void 0;
const alwaysTrue = () => true;

export type ExecutionFunc<T = unknown> = (params?: RequestParams, otherOptions?: Record<string, unknown>) => Promise<T>;
...
export function createDataSourceItem(
  ...
  const hooks = {
    ...
    dataHandler: dataHandler ? parser.parseSchema(dataHandler, scope) : (res: Response) => res.data,
...
export function createDataSourceManager({ list = [], dataHandler }: InterpretDataSource, scope: RuntimeScope) {
   ...
   */
  const reloadDataSource = (id?: string, params?: RequestParams, otherOptions?: Record<string, unknown>) => {
    if (id) {
