import type { IPublicTypeNodeSchema } from '@stall-lowcode/types';
import { h, shallowRef, mergeProps, type InjectionKey, inject, provide, watch, Fragment } from 'vue';
import { onUnmounted, defineComponent } from 'vue';
import { leafProps } from './base';
import { buildSchema, isFragment, splitLeafProps, useLeaf, type SlotSchemaMap } from './use';
import { useRendererContext } from '@stall-lowcode/hooks';
import { debounce, exportSchema, isJSSlot } from '@stall-lowcode/utils';
...
export const Hoc = defineComponent({
  ...
  setup(props, { slots, attrs }) {
    ...
    const { locked, node, buildSlots, getNode, isRootNode } = useLeaf(props, (schema, show) => {
      const id = schema.id;
      if (id) {
