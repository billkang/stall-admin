import { isFunction, isPlainObject, noop } from '@stall-lowcode/utils';
import { computed, type ComputedOptions } from 'vue';
import { AccessTypes, addToScope, type RuntimeScope, type SchemaParser } from '../../utils';

export function initComputed(parser: SchemaParser, schema: unknown, scope: RuntimeScope): void {
  ...
  for (const key in options) {
    ...
    const get = isFunction(computedOptions)
      ...
      ? computedOptions.get
      : noop;
    const set = !isFunction(computedOptions) && isFunction(computedOptions.set) ? computedOptions.set : noop;
