import { camelCase, isArray, isFunction, isObject, isString } from '@stall-lowcode/utils';
...
export function initProps(parser: SchemaParser, schema: unknown, scope: RuntimeScope): void {
  ...
  for (const key in propsConfig) {
    ...
    if (isString(opt)) {
      ...
      prop = isArray(opt) || isFunction(opt) ? { type: opt } : (opt as Record<string, any>);
    }
  ...
  if (Object.keys(propsOptions).length > 0) {
    scope.$.propsOptions = [{ ...rawPropsOptions, ...propsOptions }, [...rawNeedCastKeys, ...needCastKeys]];
  }
}
