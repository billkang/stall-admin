import { isArray, isObject } from '@stall-lowcode/utils';
import { inject, isRef, type Ref } from 'vue';
import { AccessTypes, addToScope, type RuntimeScope, type SchemaParser } from '../../utils';

export function initInject(parser: SchemaParser, schema: unknown, scope: RuntimeScope): void {
  ...
  for (const key in normalizedOptions) {
    ...
    if (isObject(opt)) {
      ...
      if ('default' in opt) {
        injected = inject(injectionKey, opt.default, true /* treat default function as factory */);
      } else {
