import { isFunction, isObject, isContainerSchema, isESModule, isPromise, sleep } from '@stall-lowcode/utils';
import type { ComponentPublicInstance } from 'vue';
import type { Router, RouteComponent, NavigationGuardWithThis, RouteRecordNormalized } from 'vue-router';
...
export function setupLowCodeRouteGuard(router: Router, options?: SetupLowCodeRouteGuardOptions) {
    ...
    Promise.all(
      to.matched.map(async route => {
        ...
        if (defaultView && isContainerSchema(schema)) {
          ...
          components.default = wrapRouteComponentGuard(route, addedView, schema, parser.initModule(schema));
        }
