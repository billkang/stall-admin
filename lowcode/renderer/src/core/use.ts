...
import { type MaybeArray, type BlockScope, type RuntimeScope, SchemaParser, warnOnce, warn } from '../utils';
import { getCurrentNodeKey, useRendererContext } from '@stall-lowcode/hooks';
import {
  camelCase,
  isNil,
  isString,
...
export function isReactLifecycleKey(key: string): key is keyof typeof REACT_ADATPOR_LIFTCYCLES_MAP {
...
export function useLeaf(
      ...
      if (!comp) {
        ...
        comp = {
          setup(props, { slots }) {
            ...
            return h('div', mergeProps(props, { class: 'lc-component-not-found' }), slots);
          },
        };
      }
    }
   ...
   */
  const buildSlots = (slots: SlotSchemaMap, node?: Node | null, blockScope?: BlockScope | null): Slots => {
    return Object.keys(slots).reduce(
      (prev, next) => {
        ...
        if (isDefaultSlot && !node?.isContainerNode && isArray(slotSchema) && slotSchema.length === 0) return prev;
        ...
        if (isArray(slotSchema)) {
          ...
          if (slotSchema.id) {
            ...
            renderSlot = (...args: unknown[]) => {
              const vnode = renderComp(slotSchema, [blockScope, parser.parseSlotScope(args, slotSchema.params ?? [])]);
  ...
  const buildNormalProp = (
    ...
    if (isJSExpression(schema) || isJSFunction(schema)) {
      ...
      return schema.map((item, idx) => buildNormalProp(item, scope, blockScope, prop?.get(idx)));
    ...
    Object.keys(processed).forEach(propName => {
      ...
      parsedProps[propName] =
        propName === 'ref'
          ...
          : buildNormalProp(schema, mergedScope, blockScope, node?.getProp(propName) as Prop);
    });
  ...
  const buildShow = (schema: NodeSchema, scope: RuntimeScope, isDesignMode: boolean) => {
    ...
    return {
      scence: 'condition',
      show: typeof condition === 'boolean' ? condition : !!parser.parseSchema(condition, scope),
    };
  };
...
export function useRootScope(rendererProps: RendererProps, setupConext: object) {
  ...
  const { props: propsSchema, state: stateSchema, methods: methodsSchema, lifeCycles: lifeCyclesSchema } = schema ?? {};
  ...
  // 处理 dataSource
  const { dataSource, dataSourceMap, reloadDataSource, hasInitDataSource } = createDataSourceManager(
...
const processProp = (target: Record<string, unknown>, key: string, val: unknown) => {
  if (key.startsWith('v-model')) {
    ...
    if (isJSExpression(val)) {
      ...
      target[eventProp] = eventProp in target ? ensureArray(target[eventProp]).concat(updateEventFn) : updateEventFn;
    }
