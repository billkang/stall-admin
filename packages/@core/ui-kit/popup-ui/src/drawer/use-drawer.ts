...
import StallDrawer from './drawer.vue';

const USER_DRAWER_INJECT_KEY = Symbol('STALL_DRAWER_INJECT');

export function useStallDrawer<
...
>(options: DrawerApiOptions = {}) {
  ...
  const Drawer = defineComponent(
    (props: DrawerProps, { attrs, slots }) => {
      return () =>
        h(StallDrawer, { ...props, ...attrs, drawerApi: extendedApi }, slots);
    },
    {
      inheritAttrs: false,
      name: 'StallDrawer',
...
async function checkProps(api: ExtendedDrawerApi, attrs: Record<string, any>) {
  ...
  for (const attr of Object.keys(attrs)) {
    if (stateKeys.has(attr) && !['class'].includes(attr)) {
      // connectedComponent存在时，不要传入Drawer的props，会造成复杂度提升，如果你需要修改Drawer的props，请使用 useStallDrawer 或者api
      console.warn(
        `[Stall Drawer]: When 'connectedComponent' exists, do not set props or slots '${attr}', which will increase complexity. If you need to modify the props of Drawer, please use useStallDrawer or api.`,
      );
    }
  }
}
