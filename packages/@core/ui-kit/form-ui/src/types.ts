...
import type { StallButtonProps } from '@stall-core/shadcn-ui';
import type { ClassType } from '@stall-core/typings';
...
export interface ActionButtonOptions extends StallButtonProps {
  [key: string]: any;
  content?: string;
  show?: boolean;
}
...
export type ExtendedFormApi = FormApi & {
  useStore: <T = NoInfer<StallFormProps>>(
    selector?: (state: NoInfer<StallFormProps>) => T,
  ) => Readonly<Ref<T>>;
};

export interface StallFormAdapterOptions<
  T extends BaseFormComponentType = BaseFormComponentType,
> {
  config?: {
