...
import type { Recordable } from '@stall-core/typings';

import type { FormActions, FormSchema, StallFormProps } from './types';

import { toRaw } from 'vue';

import { Store } from '@stall-core/shared/store';
import {
  bindMethods,
  createMerge,
  isDate,
  ...
  StateHandler,
} from '@stall-core/shared/utils';

function getDefaultState(): StallFormProps {
  return {
    actionWrapperClass: '',
    collapsed: false,
    collapsedRows: 1,
...
export class FormApi {
  // private api: Pick<StallFormProps, 'handleReset' | 'handleSubmit'>;
  public form = {} as FormActions;
  isMounted = false;

  public state: null | StallFormProps = null;
  stateHandler: StateHandler;

  public store: Store<StallFormProps>;

  // 最后一次点击提交时的表单值
  private latestSubmissionValues: null | Recordable<any> = null;

  private prevState: null | StallFormProps = null;

  constructor(options: StallFormProps = {}) {
    const { ...storeState } = options;

    const defaultState = getDefaultState();

    this.store = new Store<StallFormProps>(
      {
        ...defaultState,
        ...storeState,
      },
  ...
  setState(
    stateOrFn:
      | ((prev: StallFormProps) => Partial<StallFormProps>)
      | Partial<StallFormProps>,
  ) {
  ...
  private async getForm() {
    ...
    if (!this.form?.meta) {
      throw new Error('<StallForm /> is not mounted');
    }
    return this.form;
  }
