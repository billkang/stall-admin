...
import { STALL_DOC_URL } from '@stall/constants';

import { StallRenderContent } from '@stall-core/shadcn-ui';
...
declare global {
  const __STALL_ADMIN_METADATA__: {
    authorEmail: string;
    authorName: string;
    authorUrl: string;
    buildTime: string;
...
const renderLink = (href: string, text: string) =>
  h(
    'a',
    { href, target: '_blank', class: 'stall-link' },
    { default: () => text },
  );
...
} = __STALL_ADMIN_METADATA__ || {};

const stallDescriptionItems: DescriptionItem[] = [
  {
    content: version,
    title: '版本号',
  },
  ...
  {
    content: renderLink(STALL_DOC_URL, '点击查看'),
    title: '文档地址',
  },
  {
    content: h('div', [
    ...
    <div class="card-box p-5">
      ...
      <div class="mt-4">
        <dl class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
          <template v-for="item in stallDescriptionItems" :key="item.title">
            <div class="border-border border-t px-4 py-6 sm:col-span-1 sm:px-0">
              ...
              <dd class="text-foreground mt-1 text-sm leading-6 sm:mt-2">
                <StallRenderContent :content="item.content" />
              </dd>
            </div>
          </template>
        </dl>
