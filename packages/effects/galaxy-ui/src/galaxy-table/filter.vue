...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    const displayMoreFilter = computed(() => {
      return (
        filterableColumns.value.some(
          (col: TableColumnData) =>
            col.filterable?.componentType !== 'input' ||
            col.filterable?.filters,
        ) || inputSearchColumns.value.length > 1
      );
    });

    const handleChangeMode = (mode: 'card' | 'table') => {
