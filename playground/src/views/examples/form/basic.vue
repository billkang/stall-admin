...
import { Page } from '@stall/common-ui';
...
const [BaseForm, baseFormApi] = useStallForm({
  // 所有表单项共用，可单独在表单内覆盖
  commonConfig: {
    // 在label后显示一个冒号
    colon: true,
...
const [CustomLayoutForm] = useStallForm({
  // 所有表单项共用，可单独在表单内覆盖
  commonConfig: {
    // 所有表单项
    componentProps: {
...
<template>
  ...
  >
    ...
    <template #extra>
      <DocButton class="mb-2" path="/components/common-ui/stall-form" />
    </template>
