...
import { Page, useStallModal } from '@stall/common-ui';
...
const [BaseModal, baseModalApi] = useStallModal({
  // 连接抽离的组件
  connectedComponent: BaseDemo,
});

const [InContentModal, inContentModalApi] = useStallModal({
  // 连接抽离的组件
  connectedComponent: InContentModalDemo,
});

const [AutoHeightModal, autoHeightModalApi] = useStallModal({
  connectedComponent: AutoHeightDemo,
});

const [DragModal, dragModalApi] = useStallModal({
  connectedComponent: DragDemo,
});

const [DynamicModal, dynamicModalApi] = useStallModal({
  connectedComponent: DynamicDemo,
});

const [SharedDataModal, sharedModalApi] = useStallModal({
  connectedComponent: SharedDataDemo,
});

const [FormModal, formModalApi] = useStallModal({
...
<template>
  ...
  >
    <template #extra>
      <DocButton path="/components/common-ui/stall-modal" />
    </template>
    <BaseModal />
    <InContentModal />
    <AutoHeightModal />
